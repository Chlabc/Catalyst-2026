...
const WELCOME_KEY = "blossom_has_seen_welcome";
  ...
  return (
    ...
    >
      <section className="relative w-full max-w-lg overflow-hidden rounded-[2rem] border-2 border-primary/25 bg-primary-soft p-7 shadow-2xl sm:p-9">
        <div aria-hidden className="absolute -right-8 -top-9 text-8xl opacity-80">🌼</div>
        <div aria-hidden className="absolute -bottom-12 -left-8 h-36 w-36 rounded-full bg-secondary/25" />
        <div className="relative">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary-dark">Welcome to Blossom</p>
          <h1 id="welcome-title" className="mt-3 max-w-sm text-4xl font-semibold leading-tight tracking-tight text-foreground sm:text-5xl">
          ...
          <p className="mt-4 max-w-md text-base leading-relaxed text-foreground/75 sm:text-lg">
            Learn useful things in little bites, practise real-life moments, and build an island that feels like yours.
          </p>
          ...
          <div className="mt-6 grid gap-2 sm:grid-cols-3">
            {[
              ...
              <div key={number} className="rounded-2xl bg-white/70 p-3">
                <span className="flex h-6 w-6 items-center justify-center rounded-full bg-secondary text-xs font-bold text-white">{number}</span>
          ...
          <p className="mt-4 text-center text-xs text-text-muted">No account needed. Your tracker stays on this device.</p>
        </div>
      </section>
    </div>
  );
